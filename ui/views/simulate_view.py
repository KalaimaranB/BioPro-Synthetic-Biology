...
class SimulateView(QWidget):
    ...
    def _apply_theme(self):
        try:
            ...
            self.info_label.setStyleSheet(f"color: {Colors.FG_SECONDARY}; font-size: {Fonts.SIZE_SMALL + 2}px;")
        except ImportError:
    ...
    def plot_steady_state(self):
        ...
        if not promoters:
            ax.text(0.5, 0.5, "No Promoters found to plot transfer curve.", 
    ...
    def plot_time_series(self, max_time: int = 1000, method: str = 'ode'):
        ...
        try:
            ...
            for col in result.colnames[1:]:
                ...
                ax.plot(result['time'], result[col], label=col.replace('[', '').replace(']', ''), linewidth=lw)
                
            ax.set_xlabel('Time (seconds)', color='white')
            ax.set_ylabel('Concentration' if method == 'ode' else 'Molecule Count', color='white')
