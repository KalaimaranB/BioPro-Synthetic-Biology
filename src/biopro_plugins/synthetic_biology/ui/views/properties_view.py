    ...
    def _populate_inspector(self, part):
            ...
            if "http" in citation:
                ...
                if url_match:
                    ...
                    citation = citation.replace(
                        url, f"<a href='{url}' style='color: #8b949e;'>[Link]</a>"
                    )
            source_label = QLabel(
                f"<small style='color: #8b949e;'><i>Source: {citation}</i></small>"
            )
        ...
        if isinstance(part, Promoter):
            self._add_double_spinbox(
                "y_min (Leakiness in RPU):", "y_min", part.y_min, 0.0, 10.0, 0.01
            )
            self._add_double_spinbox(
            ...
            self._add_double_spinbox(
                ...
                part.translation_initiation_rate,
                0.0,
                10.0,
                0.1,
            )
