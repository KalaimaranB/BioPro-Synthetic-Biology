...
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QPushButton, QTextEdit, QLabel, QComboBox, QSizePolicy, QMessageBox,
...
class ModelDetailsDialog(QDialog):
    ...
    def _generate_html_content(self) -> str:
        if "thermodynamic pwm" in self.model_key or "promoter" in self.model_key:
            ...
            """
        elif "cai" in self.model_key or "blosum62" in self.model_key or "cds" in self.model_key:
            return f"""
...
class PartInspector(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.prediction_status_lbl.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        ...
        self.prediction_info_card.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        ...
        self.predict_btn.setStyleSheet(f"background: {Colors.BG_MEDIUM}; color: {Colors.ACCENT_PRIMARY}; border: 1px solid {Colors.ACCENT_PRIMARY}; font-weight: bold; padding: 6px;")
        ...
        self.seq_graph_btn.setStyleSheet(f"background: {Colors.ACCENT_PRIMARY}; color: {Colors.BG_DARKEST}; font-weight: bold; padding: 6px; margin-top: 4px;")
        ...
        self.save_btn.setStyleSheet(f"background: {Colors.ACCENT_PRIMARY}; color: {Colors.BG_DARKEST}; font-weight: bold; padding: 6px;")
        self.save_btn.clicked.connect(self._on_save)
        self.btn_layout.addWidget(self.save_btn)
        
        self.delete_btn = QPushButton("Delete Part")
        self.delete_btn.setStyleSheet(f"background: {Colors.ACCENT_PRIMARY}; color: {Colors.BG_DARKEST}; font-weight: bold; padding: 6px;")
        ...
        if self.catalogue_service:
            ...
            try:
                ...
                try:
                    ...
                    from biopro.plugins.synthetic_biology.analysis.prediction.sequence_predictor import SequencePredictor
        ...
        try:
            try:
                ...
                try:
                    ...
                    from biopro.plugins.synthetic_biology.analysis.prediction.sequence_predictor import compare_kinetics
                    from biopro.plugins.synthetic_biology.analysis.prediction.graphing_utils import generate_transfer_curve
