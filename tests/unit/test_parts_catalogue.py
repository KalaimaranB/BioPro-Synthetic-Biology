...
def test_service_initializes_cello_parts(tmp_path):
    ...
    assert "laci regulated" in bba.description.lower(), (
        f"Description not populated for BBa_R0010: {bba.description}"
    )
    ...
    assert "Cello UCF" in pamtr.description, (
        f"Description not populated for pAmtR: {pamtr.description}"
    )
    assert len(pamtr.sequence) > 10, "Sequence not extracted from UCF for pAmtR"


def test_service_delete_part(tmp_path):
    repo_path = tmp_path / "catalogue.json"
    repo = JsonPartRepository(str(repo_path))
    service = PartsCatalogueService(repo)

    part = Promoter(id="custom_p1", name="Custom Promoter 1")
    service.add_part(part)

    assert service.get_part("custom_p1") is not None
    service.delete_part("custom_p1")
    assert service.get_part("custom_p1") is None
